...
```mermaid
graph LR
    A[Business Transaction] --> B[Record Entry]
    B --> C[Post to Accounts]
    C --> D[Prepare Trial Balance]
    D --> E[Financial Statements]
    E --> F[Analysis & Decisions]
    F --> G[New Business Activities]
    G --> A
```
...
```mermaid
graph LR
    A[Upstream Activities] --> B[Reporting Company]
    B --> C[Downstream Activities]
    
    A1[Suppliers<br/>Raw Materials] -.-> A
    A2[Manufacturing<br/>Components] -.-> A
...
```mermaid
graph TD
    A[Stakeholders Provide Resources] --> B[Management Controls Resources]
    B --> C[Accounting Reports Activities]
    C --> D[Stakeholders Make Decisions]
    D --> E[Good Decisions Lead to Value Creation]
    E --> A
