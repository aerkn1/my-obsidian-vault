...
```mermaid
graph TD
    A[Accurate Accounting] --> B[Informed Decisions]
    B --> C[Efficient Capital Allocation]
    C --> D[Economic Growth]
    
    E[Fraudulent Accounting] --> F[Misled Stakeholders]
    F --> G[Bad Decisions]
    G --> H[Capital Destruction]
    H --> I[Economic Crisis]
...
graph TD
    ...
    B[PRESSURE<br/>Need for results] --> A
    C[OPPORTUNITY<br/>Weak controls] --> A
    D[RATIONALIZATION<br/>Justification] --> A
...
```mermaid
graph TD
    A[Prevention & Detection] --> B[Segregation of Duties]
    A --> C[Authorization Procedures]
    A --> D[Documentation]
    A --> E[Physical Controls]
    A --> F[Reconciliations]
    
    B --> G[Different people:<br/>authorize, record, custody]
    C --> H[Approval required<br/>for transactions]
    D --> I[Audit trail<br/>of all activities]
    E --> J[Safeguards for<br/>assets & records]
    F --> K[Regular checks<br/>& balances]
...
```mermaid
graph TD
    A[You discover an error<br/>that overstates profit] --> B{What do you do?}
    
    B --> C[Report it honestly]
    B --> D[Stay quiet]
    
    C --> E[Short-term consequences:<br/>- Boss unhappy<br/>- Profit revised down<br/>- Possible job risk]
    C --> F[Long-term consequences:<br/>- Integrity intact<br/>- Legal protection<br/>- Professional reputation]
    
    D --> G[Short-term consequences:<br/>- Boss happy<br/>- Keep your job<br/>- Avoid conflict]
    D --> H[Long-term consequences:<br/>- Complicit in fraud<br/>- Legal liability<br/>- Career destroyed<br/>- Possible jail]
...
```mermaid
graph LR
    A[Receive Request] --> B[Assess Appropriateness]
    B --> C{Acceptable?}
    
    C -->|Yes| D[Proceed & Document]
    C -->|No| E[Document Objection]
    
    E --> F[Discuss with Superior]
    F --> G{Resolved?}
    
    G -->|Yes| H[Document Resolution]
    G -->|No| I[Escalate Further]
    
    I --> J[Document All Steps]
    J --> K[Consider Legal Advice]
