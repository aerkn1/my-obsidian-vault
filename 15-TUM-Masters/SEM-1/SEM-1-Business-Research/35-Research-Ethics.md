# Research Ethics and Integrity

## 🎯 Doing Research Responsibly

Ethics are fundamental to research credibility and trustworthiness.

---

## 📋 Core Principles

### 1. Respect for Persons
- Informed consent
- Autonomy
- Special protection for vulnerable populations

### 2. Beneficence
- Maximize benefits
- Minimize harms
- Risk-benefit analysis

### 3. Justice
- Fair distribution of benefits/burdens
- Equitable selection

---

## ✅ Informed Consent

**Elements**:
- Purpose of research
- Procedures
- Risks and benefits
- Voluntary participation
- Right to withdraw
- Confidentiality

---

## 🚨 Research Misconduct

### Fabrication
Making up data

### Falsification
Manipulating data/results

### Plagiarism
Using others' work without credit

---

## ⚠️ Consequences

**For Science**:
- Erodes trust
- Wastes resources
- Misleads policy

**For Researchers**:
- Career damage
- Retraction of papers
- Legal consequences

**For Participants**:
- Physical/psychological harm
- Breach of trust

---

## 🔑 Key Takeaways

1. **Ethics**: Fundamental to research
2. **Informed consent**: Required
3. **Misconduct**: Fabrication, falsification, plagiarism
4. **Consequences**: Severe for all parties
5. **IRB**: Review board ensures ethics

---

*Part of: [[15-TUM-Masters/SEM-1/SEM-1-Business-Research/00-Index|Business Research Methods Course Notes]]*
